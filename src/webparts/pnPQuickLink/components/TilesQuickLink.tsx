...
import { TileSize, LinkAlignment } from "../models/enums"; // Added LinkAlignment
...
export interface ITilesQuickLinksProps extends IPnPQuickLinksProps {
  ...
  hideText: boolean;
  borderRadius: number;
  linkAlignment?: LinkAlignment; // Added linkAlignment
}

export const TilesQuickLinks: React.FunctionComponent<ITilesQuickLinksProps> = (
  props: React.PropsWithChildren<ITilesQuickLinksProps>
) => {
  const {
    displayMode,
    webPartTitle,
  ...
  const SortableList = SortableContainer(({ items }: { items: ILink[] }) => (
    <div
      style={{
        ...
        display: "flex",
        flexWrap: "wrap",
        justifyContent: linkAlignment || LinkAlignment.Left,
      }}
    >
